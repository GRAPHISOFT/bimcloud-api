...
class Workflow:
	...
	def run(self):
		...
		try:
			...
			# test = self._manager_api.get_items_by_criterion(self._auth_context, 'permissions')
			test = self._manager_api.get_permissions_by_name(self._auth_context, 'ArchiCAD - External Content', '_server.archicad.permission.library.part.create')
