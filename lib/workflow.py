...
class Workflow:
	...
	def run(self):
		...
		try:
			...
			# test = self._manager_api.rename_resource_backup(self._auth_context, '9092251A-F027-4F78-88AA-8C341A94B0E8', '9671af7f-2a80-9d1d-88ff-b3f1dbd8602f9092251A-F027-4F78-88AA-8C341A94B0E8_server.backup.format.bimprojectDC6DDE9A-55F1-454A-B14C-413F96711923', 'Python API Test')
			test = self._manager_api.get_resource_backups_by_criterion(self._auth_context, ['9092251A-F027-4F78-88AA-8C341A94B0E8',])
			# test = self._manager_api.get_items_by_criterion(self._auth_context, 'backup')
			# test = self._manager_api.remove_all_tags(self._auth_context)
			# test = self._manager_api.create_resource_backup(self._auth_context, '9092251A-F027-4F78-88AA-8C341A94B0E8', 'bimproject', 'Python API')
			# test = self._manager_api.insert_private_tag(self._auth_context, 'bim', '#0077FF')
